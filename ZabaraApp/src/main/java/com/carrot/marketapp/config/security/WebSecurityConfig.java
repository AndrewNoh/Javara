...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(AuthenticationManagerBuilder auth, HikariDataSource dataSource) throws Exception {
		auth.jdbcAuthentication().dataSource(dataSource)
				.usersByUsernameQuery("SELECT m.email as username,m.password,enabled FROM members m JOIN AUTHORITY_SECURITY AU ON m.userno=au.userno WHERE m.email=?")
				.authoritiesByUsernameQuery("SELECT m.email as username,authority FROM members m JOIN AUTHORITY_SECURITY AU ON m.userno=au.userno WHERE m.email=?");
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests() // 요청에 의한 보안검사 시작
				.antMatchers("/board/**", "/auction/**", "/chat/**").hasAnyRole("USER")
				...
				.and().logout().logoutUrl("/security/logout.do").logoutSuccessUrl("/")
